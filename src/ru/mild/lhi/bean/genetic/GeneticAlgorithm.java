...
public class GeneticAlgorithm {
    ...
    public GeneticAlgorithm(Graph graph, Vertex start, Vertex end, int populationSize, double mutationRate, double crossoverRate, int elitismCount) {
    ...
    public double calcFitness(Individual individual) {
        ...
        String individualSolution = individual.toString();
        ...
        for (int i = 0; i < individualSolution.length(); i++) {
            if (i % 2 == 0) {
                ...
                switch (Graph.Direction.fromCode(code)) {
                    case DOWN: {
                        got = graph.getVertexIfItsFloor(currentVertex.getX(), currentVertex.getY() + 1);
                    }
                    break;
                    case LEFT: {
                        got = graph.getVertexIfItsFloor(currentVertex.getX() - 1, currentVertex.getY());
                    }
                    break;
                    case RIGHT: {
                        got = graph.getVertexIfItsFloor(currentVertex.getX() + 1, currentVertex.getY());
